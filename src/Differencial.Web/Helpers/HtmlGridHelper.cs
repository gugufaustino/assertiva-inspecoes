...
namespace Differencial.Web.Helpers
{
	public static class HtmlGridHelper
	{
		...
		public static HtmlString IndPendenteConcluido(bool indicador, DateTime? dthConcluido = null)
		{
			string classSpan, tpSituacao  ; 
			...
			string shtml = string.Format("<span class=\"label {0}\">{1}</span><br />", classSpan, tpSituacao);
			if (dthConcluido.HasValue)
			{ 
				shtml += string.Format("<small class=\"text-nowrap\">{0}</small>", dthConcluido.FormatoDataHora());
			}
			return new HtmlString(shtml);
		} 
